...
#define FUNSTATIC(var, def) \
    static decltype(def) var; \
    if (!var) var = def;
...
public:
	...
	struct Type final {
		...
		std::string assembly_qualified_name;
		int         size;
	...
	static auto DumpToFile(const std::string& path) -> void {
		std::cout << "Try open " << path << "dump.cs" << std::endl;

		std::ofstream io(path + "dump.cs", std::fstream::out);
		if (!io) return;

		std::cout << "Assemblies " << assembly.size() << std::endl;
	...
	static auto ForeachClass(Assembly* assembly, void* image) -> void {
		...
		if (mode_ == Mode::Il2Cpp) {
			...
			for (auto i = 0; i < count; i++) {
				...
				const auto pAClass = new Class{ .parent_type = new Type{} };
				...
				if (const auto pPClass = Invoke<void*>("il2cpp_class_get_parent", pClass)) {
					...
					pAClass->parent_type->address = Invoke<void*>("il2cpp_class_get_type", pPClass);
					pAClass->parent_type->name = Invoke<const char*>("il2cpp_type_get_name", pAClass->parent_type->address);
					pAClass->parent_type->reflection_name = Invoke<const char*>("il2cpp_type_get_reflection_name", pAClass->parent_type->address);
					pAClass->parent_type->assembly_qualified_name = Invoke<const char*>("il2cpp_type_get_assembly_qualified_name", pAClass->parent_type->address);
		...
		if (mode_ == Mode::Il2Cpp) {
			...
			do {
				if ((field = Invoke<void*>("il2cpp_class_get_fields", pKlass, &iter))) {
					...
					pField->type->reflection_name = Invoke<const char*>("il2cpp_type_get_reflection_name", pField->type->address);
					pField->type->assembly_qualified_name = Invoke<const char*>("il2cpp_type_get_assembly_qualified_name", pField->type->address);
					pField->type->size = -1;
		...
		if (mode_ == Mode::Il2Cpp) {
			...
			do {
				if ((method = Invoke<void*>("il2cpp_class_get_methods", pKlass, &iter))) {
					...
					pMethod->return_type->reflection_name = Invoke<const char*>("il2cpp_type_get_reflection_name", pMethod->return_type->address);
					pMethod->return_type->assembly_qualified_name = Invoke<const char*>("il2cpp_type_get_assembly_qualified_name", pMethod->return_type->address);
...
public:
	...
	public:
		...
		struct String : Object {
			...
			[[nodiscard]] auto ToCString() const -> std::string {
#if WINDOWS_MODE
				if (IsBadReadPtr(this, sizeof(String))) return {};
				if (IsBadReadPtr(m_firstChar, m_stringLength)) return {};
#endif
				if (!this) return {};
				try {
					std::vector<char> buffer(m_stringLength + 1);
					std::wcstombs(buffer.data(), m_firstChar, buffer.size());
		...
		struct UnityObject : Object {
			...
			auto GetName() -> std::string {
				...
				if (method) return method->Invoke<String*>(this)->ToCString();
				return {};
			}
			...
			auto GetInstanceID() -> int32_t {
				if (!this) return {};
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Object")->Get<Method>("GetInstanceID");
				if (method) return method->Invoke<int32_t>(this);
				return {};
			}
			...
			static auto FindObjectsOfType(const std::string strAsmembly, const std::string strClass) -> std::vector<UnityObject*> {
				FUNSTATIC(assembly, UnityResolve::Get(strAsmembly)->Get(strClass));
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Object")->Get<Method>("FindObjectsOfType", { "System.Type" }));
				
				if (assembly && method) return method->Invoke<Array<UnityObject*>*>(assembly->GetType())->ToVector();
				return {};
			}

			template<typename T>
			static auto FindObjectsOfType(const std::string strAsmembly = "UnityEngine.CoreModule.dll") -> std::vector<T*> {

				std::string str = std::string(std::type_index(typeid(T)).name());
				size_t pos = str.find_last_of("::");
				std::string strClass;
				if (pos != std::string::npos) {	strClass = str.substr(pos + 1); }
				else { strClass = str; }
				...
				if (assembly && method) return method->Invoke<Array<T*>*>(assembly->GetType())->ToVector();
				return {};
			}
		...
		struct Scene {
			...
			auto IsValid() -> bool {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("IsValidInternal", { "System.Int32" }));
				if (method && m_Handle != 0) return method->Invoke<bool>(m_Handle);
				return false;
			}

			auto GetName() -> std::string {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetNameInternal", { "System.Int32" }));
				if (method && m_Handle != 0) return method->Invoke<String*>(m_Handle)->ToCString();
				return {};
			}

			auto GetIsLoaded() -> bool {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetIsLoadedInternal", { "System.Int32" }));
				if (method && m_Handle != 0) return method->Invoke<bool>(m_Handle);
				return {};
			}

			auto GetBuildIndex() -> int32_t {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetBuildIndexInternal", { "System.Int32" }));
				if (method && m_Handle != 0) return method->Invoke<int32_t>(m_Handle);
				return {};
			}

			auto GetRootCount() -> int32_t {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetRootCountInternal", { "System.Int32" }));
				if (method && m_Handle != 0) return method->Invoke<int32_t>(m_Handle);
				return {};
			}

			auto GetRootGameObjects() -> std::vector<GameObject*> {
				FUNSTATIC(klass, Get("UnityEngine.CoreModule.dll")->Get("GameObject"));
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetRootGameObjectsInternal", { "System.Int32", "*" }));

				auto lst = UnityResolve::UnityType::List<GameObject*>::New(klass, GetRootCount());
				if (method && m_Handle != 0) method->Invoke<void>(m_Handle, lst);

				auto vec = lst->ToArray()->ToVector();
				delete lst;
				return vec;
			}

			static auto GetNameInternal(int32_t handle) -> std::string {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("Scene")->Get<Method>("GetNameInternal", { "System.Int32" }));
				if (method) return method->Invoke<String*>(handle)->ToCString();
				return {};
			}
		};

		struct SceneManager : UnityObject {
			static auto GetSceneCount() -> uint32_t {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("SceneManager")->Get<Method>("get_sceneCount");
				if (method) return method->Invoke<uint32_t>();
				return 0;
			}

			static auto GetSceneAt(uint32_t index) -> Scene* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("SceneManager")->Get<Method>("GetSceneAt");
				if (method) return method->Invoke<Scene*>();
				return nullptr;
			}

			static auto GetActiveScene_Injected() -> int32_t {
				FUNSTATIC(method, Get("UnityEngine.CoreModule.dll")->Get("SceneManager")->Get<Method>("GetActiveScene_Injected", { "*" }));

				int32_t handle = 0;
				if (method) method->Invoke<void>(&handle);
				return handle;
			}
